...
const { secondsUntilNextPurge } = require("./utils");
...
function serverCollisionCheck(req, serverArray) {
  if (!req.serverUuid) {
    ...
    const existingServer = serverArray.list.find(
      (server) => server.ip === req.ip && server.port === +req.body.serverPort
    );

    if (existingServer) {
      if (configuration.Security.sendNextPurgeTimeInSeconds) {
        const seconds = secondsUntilNextPurge(
          serverArray.timeLastPurged,
